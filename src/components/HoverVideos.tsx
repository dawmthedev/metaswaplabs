...
const CustomCursor: React.FC<CustomCursorProps> = ({ position, isVisible, isHovering, videoSrc }) => {
  const videoRef = useRef<HTMLVideoElement>(null);

  useEffect(() => {
    if (videoRef.current && videoSrc) {
      const absoluteVideoSrc = videoSrc.startsWith('/') ? window.location.origin + videoSrc : videoSrc;
      if (videoRef.current.src !== absoluteVideoSrc) {
        videoRef.current.src = absoluteVideoSrc;
        videoRef.current.load();
        videoRef.current.play().catch(error => console.error("Cursor video play failed:", error));
      }
    }
  }, [videoSrc]);
...
const HoverVideos: React.FC<HoverVideosProps> = ({ items }) => {
  ...
  useEffect(() => {
    if (items?.length) {
      const webExpIndex = items.findIndex(item => item.title.toLowerCase().includes('web experiment'));
      const initialIndex = webExpIndex !== -1 ? Math.max(0, webExpIndex - 3) : 0;
  ...
  useEffect(() => {
    if (isMobile && mobileVideoRef.current && currentVideoSrc) {
      ...
      if (mobileVideoRef.current.src !== window.location.origin + newSrc) {
        ...
        mobileVideoRef.current.play().catch(error => console.error("Mobile video play failed:", error));
      }
    }
    ...
    >
      ...
      {isMobile ? (
        <div className="grid grid-cols-1">
          ...
          <div className="col-start-1 row-start-1 z-10 mx-auto max-w-7xl px-6 pb-[100vh]">
          ...
          <div className="col-start-1 row-start-1 z-20 flex h-full items-start justify-center">
